...
namespace ArnNavigation.Application.Services
{
    public sealed class CarsService : ICarsService
    {
        ...
        public Task<IEnumerable<Car>> SearchAsync(string query, int requesterRole, Guid requesterOrgId, Guid? orgId, CancellationToken ct)
        {
            var scopeOrg = requesterRole == (int)Role.SuperAdmin ? orgId : requesterOrgId;
            return _cars.SearchAsync(query, scopeOrg, ct);
        }
